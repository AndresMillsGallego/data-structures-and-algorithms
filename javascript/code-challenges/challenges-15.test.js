...
const screenForNames = (arr) => {
  // Solution code here...
  return arr.filter(string => /^(Mr\.|Mrs\.|Ms\.|Dr\.)\s[a-zA-Z]+/.test(string));
};
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let string = '';
  // Note, I first passed in arr[0].mass below, it should have worked but it failed until I specified the actual starWarsData array.
  let lukesMass = starWarsData[0].mass;
  for (let obj of arr) {
    +obj.mass > +lukesMass ? string += `${obj.name} - `
      : string;
  }
  let punyLukeString = string.split(' ').slice(0, -2).join(' ');
  return punyLukeString;
};
...
const detectTicTacToeWin = (board) => {
  // Solution code here...
  let helpCheckGame = function (x1, y1, x2, y2, x3, y3) {
    return board[x1][y1] !== '' &&
      board[x1][y1] === board[x2][y2] && board[x2][y2] === board[x3][y3];
  };
  // All winning options below
  if (helpCheckGame(0,0,0,1,0,2)) return true;
  if (helpCheckGame(0,0,1,1,2,2)) return true;
  if (helpCheckGame(1,0,1,1,1,2)) return true;
  if (helpCheckGame(2,0,2,1,2,2)) return true;
  if (helpCheckGame(2,0,1,1,0,2)) return true;
  if (helpCheckGame(0,0,1,0,2,0)) return true;
  if (helpCheckGame(0,1,1,1,2,1)) return true;
  if (helpCheckGame(0,2,1,2,2,2)) return true;
