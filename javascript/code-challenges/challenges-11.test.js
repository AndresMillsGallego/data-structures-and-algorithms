...
function transformToLis(obj) {
  ...
  for (let [key, value] of Object.entries(obj)) {
    liItemArr.push(`<li>${key}: ${value}</li>`);
  }
  return liItemArr;
}
...
const count = (target, input) => {
  ...
  for (let arr of input) {
    let arrTotal = arr.reduce(((numTotal, num) => num === target ? numTotal + 1 : numTotal), 0);
    targetTotal += arrTotal;
  }
  return targetTotal;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let fiveToTwoArr = [];
  // short code option, but not readable:
  // fiveToTwoArr.push(arr.filter(num => num % 5 === 0 && typeof num === 'number').map(int => Math.pow(2, int)));
  // I prefer the code below, sometimes shortest isn't best.
  for (let arr of input) {
    let filteredArr = arr.filter(num => num % 5 === 0 && typeof num === 'number');
    let mappedArr = filteredArr.map(int => Math.pow(2, int));
    fiveToTwoArr.push(mappedArr);
  }
  return fiveToTwoArr;
};
...
let findMaleAndFemale = (data) => {
  ...
  let string = '';
  for (let obj of data) {
    obj.gender === 'male' || obj.gender === 'female' ? string += `${obj.name} and `
      : string;
  }
  let mayTheFourthString = string.split(' ').slice(0, -2).join(' ');
  return mayTheFourthString;
};
